...
def predict_price(year, town, flat_type, flr_area_sqm, flat_model, stry_start, stry_end, les_coms_dt):
    ...
    # Ensure storey start and storey end values are positive and greater than zero
    if stry_start <= 0 or stry_end <= 0:
        st.error("Storey values must be positive and greater than zero.")
        return

    # Use np.clip to avoid log(0) or log of negative numbers
    str_str = np.log(np.clip(int(stry_start), 1, None))
    str_end = np.log(np.clip(int(stry_end), 1, None))
    
    lese_coms_dt = int(les_coms_dt)

    # Load the trained model
    with open(r"C:\Users\ABINASH\Desktop\ABINASH DS\VS Code\Resale_Flat_Prices_Model_1.pkl","rb") as f:
...
if Menu == "Home":
    ...
    if button:
        ...
        pre_price= predict_price(year, town, flat_type, flr_area_sqm, flat_model,stry_start, stry_end, les_coms_dt)
